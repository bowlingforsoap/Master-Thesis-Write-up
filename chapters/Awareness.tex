...
\end{description}
Endsley notes that the first step in gaining \gls{sa} is to "perceive the status, attributes, and dynamics of relevant elements in the environment" \cite{salvendy_copyright_nodate}. Going back to the jet fighter use case, the pilot has to keep track of all the different indicators in the cockpit. At the second level, more than the ability to take in all the need information is required, the pilot has to be able to comprehend the situation and highlight the important pieces of information with respect to the current goal. The difference between a novice and experienced pilot is that the former cannot build achieve the second level of \gls{sa}, even if they are able to get the same information on the first level. Level 3 of situation awareness describes the ability of the user to use their knowledge from the first and the second levels to predict the state of the system in the nearest future.
...
Nevertheless, the biggest limitation of the mentioned approaches was that they attempted to evaluate only a single design issue at the time. This leads Endsley to introduce the \gls{sagat} \cite{endsley_situation_1988}. The idea is that, first, the task goes though goal-directed analysis to determine the \gls{sa} requirements: the goal, subgoals, decisions required to actualize the given subgoal, and the knowledge required on all three levels of \gls{sa} (\ref{fig:sagoalorientedtaskanalysis}). 
...
This yields an extensive list of questions, which assess the \gls{sa} information of primary and secondary importance. (for an example of the results of such analysis, see the Fig. \ref{fig:sagoalorientedtaskanalysisresultexample}).
...
\paragraph[Bridge]{}
A lot of ideas from the \gls{sa} are employed and adapted in the concept of \gls{wa}.
...
The complete \gls{wa} framework is summarize in Fig. \ref{fig:waframework}.
...
\paragraph{Shared chalkboard application} Gutwin et al. \cite{gutwin_chalk_2011} employ the notion of \gls{wa} in their study about the effects of synthesized sound in a shared chalkboard \gls{cscw}.
...
%%% Setup/system
The system was a 2D shared drawing application with one real (participant) and one agent (simulated user). The agent would draw in an off-screen part of the workspace multiple times at some point during the experiment. The participant had two tasks, primary and secondary. The primary task was to trace a given 2D shape (Fig. \ref{fig:gutwinchalk2011}), and the secondary was to keep track of what is happening in the shared workspace. When the participant and the agent drew, the system would emit a spatialized sound of chalk writing on a chalkboard. Participants had an additional way to monitor the shared environment -
%, or as authors call it - a type of awareness presentation,
the minimap (\textit{radar}), which showed the complete environment and the local chunk a participant was working in (indicated with a blue rectangle on the minimap).
...
\cite{gutwin_descriptive_2002} study one main independent variable, the type of awareness presentation. To keep most of the participants attention on the shape tracing task, one of the secondary independent variables was - attentional demand. It was implemented by making the tracing shape gradually move (0, 40, or 80 pixels/second movement), and thus requiring the participant to keep track of the shapes position. The other two secondary independent variables were the workspace clutter and the size of the minimap.

The dependent measures were participants' accuracy in reporting \textit{what }type of figure the agent drew, \textit{where }and \textit{when }it drew it.
The authors use a \gls{sagat}-like approach to collecting the data in the experiment: participants were tasked with pressing a space-bar as soon, as they noticed that the agent was drawing, and report aforementioned information.
