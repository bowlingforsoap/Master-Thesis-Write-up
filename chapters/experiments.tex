...
In the experiment mode participants were not able to see the building, but would still hear the sounds when it started moving. The wall around the user was visible. The end-points of the translations of the building were always on this wall. Participants were asked to indicate where the building translated from and to with the help of a simulated laser pointer attached to the controller. The details of the controls can be found on Fig. %TODO: figure (1 - schematic controller with the controlls described like in games; 2,3,4 - how the laser pointer looks while pointing somewhere and on the wall, and what happens on-click).
...
29 out of 40 translations' means were close to the actual translation paths (about 45 degrees deviation from the actual start- and/or end-point). % Green dot
...
% Mean deviation for deviation of start and end /points // apparently, not useful for the stats analysis: https://en.wikipedia.org/wiki/Unbiased_estimation_of_standard_deviation
% Conclusion
...
after which were asked to sign the consent form for using their data. % TODO: add figures to appendix
...
The visual analysis shows that around $ \dfrac{1}{4} $th of all the translations were not caught: 11 from Group 1, 19 from Group 2, and 26 from Group 3 (Fig. \ref{fig:histograms_combined}, \ref{fig:distributions_freqReaction0}).
